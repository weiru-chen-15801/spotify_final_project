...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec  4 20:19:53 2019
...
unpopular_artist_tracks = []
unpopular_df = pd.read_csv("df_unpopular_songs.csv")
artists = list(unpopular_df['artist'])
tracks = list(unpopular_df['track_name'])
for i in range(len(artists)):
    unpopular_artist_tracks.append({'title':tracks[i],'artist':artists[i]})
...
artists = []
lyrics = []
for item in unpopular_artist_tracks:
    current_song_info = item
    curr_lyrics = get_lyrics(current_song_info)
    time.sleep(1)
...
df = pd.DataFrame({'Title': titles, 'Artist': artists, 'Lyrics': lyrics})
#df.to_csv('random_tracks_with_lyrics.csv', index=False)
df.to_csv('unpopular_with_lyrics.csv', index=False)
